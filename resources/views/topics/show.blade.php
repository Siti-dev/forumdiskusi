    ...
    <div class="container">
        <div class="card">
            <div class="card-body">
                ...
                <small>Terposting {{ $topic->created_at->format('d/m/Y') }} oleh <span class="badge badge-primary">{{ $topic->user->name }}</span></small>
                </div>
                <div class="d-flex justify-content-between align-items-center mt-3">
                    @can('update', $topic)
                    <a href="{{ route('topics.edit', $topic) }}" class="btn btn-warning">Ubah</a>
                    @endcan
                    @can('delete', $topic)
        ...
        @forelse ($topic->comments as $comment)
            <div class="card mb-2 @if($topic->solution === $comment->id) border-success @endif">
                <div class="card-body d-flex justify-content-between align-items-center">
                    ...
                    {{ $comment->content }}
                        <div class="d-flex flex-column">
                        <small>Terposting {{ $comment->created_at->format('d/m/Y') }} oleh <span class="badge badge-primary">{{ $comment->user->name }}</span></small>
                        </div>
                    </div>
                    <div>
                        @auth
                            ...
                            @else
                                @if ($topic->solution === $comment->id)
                                    <span class="badge badge-success">Ditandai sebagai solusi</span>
                                @endif
                            @endif
                        @endauth
                    </div>
            ...
            @auth
            <button class="btn btn-secondary mb-3" onclick="toggleReplyComment({{ $comment->id }})">Balas</button>
            <form action="{{ route('comments.storeReply', $comment) }}" method="POST" class="mb-3 ml-5 d-none" id="replyComment-{{ $comment->id }}">
                @csrf
                <div class="form-group">
                    <label for="replyComment">Jawaban</label>
                ...
                <button type="submit" class="btn btn-primary">Balas Komentar ini</button>
            </form>
            @endauth
            ...
            @endforeach
        @empty
            <div class="alert alert-info">Tidak ada komentar untuk topik ini</div>
        @endforelse
        @auth
        <form action="{{ route('comments.store', $topic) }}" method="POST" class="mt-3">
            ...
            <button type="submit" class="btn btn-primary">Kirimkan komentar</button>
        </form>
        @else
        <div class="alert alert-info"><a href="{{ route('login') }}">Gabung</a> untuk berpartisipasi</div>
        @endauth
    </div>
@endsection
